...
class LeopartsfilterLevel5 extends ObjectModel
{
    ...
    public function existName($val, $id_model, $id_level5, $id_lang = '', $id_shop = '')
    {
        if (!$id_lang) {
            $id_lang = Context::getContext()->language->id;
        }
        ...
        $sql = 'SELECT SQL_CALC_FOUND_ROWS level5_l.*, level5.* FROM ' . _DB_PREFIX_ . 'leopartsfilter_level5 level5
                LEFT JOIN ' . _DB_PREFIX_ . 'leopartsfilter_level5_lang level5_l ON (level5.`id_leopartsfilter_level5` = level5_l.`id_leopartsfilter_level5` AND level5_l.`id_lang` = ' . $id_lang . ')
                LEFT JOIN ' . _DB_PREFIX_ . 'leopartsfilter_level5_shop level5_s ON (level5.`id_leopartsfilter_level5` = level5_s.`id_leopartsfilter_level5` AND level5_s.`id_shop` = ' . $id_shop . ')

                WHERE level5_l.name ="' . $val . '" and level5_l.id_leopartsfilter_level5 != ' . (int) $id_level5;
        $results = Db::getInstance()->ExecuteS($sql);
        return $results;
    }
}
