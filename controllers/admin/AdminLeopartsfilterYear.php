...
class AdminLeopartsfilterYearController extends ModuleAdminControllerCore
{
    public $mod_config;
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        $this->table = 'leopartsfilter_year';
        $this->identifier = 'id_leopartsfilter_year';
        $this->className = 'LeopartsfilterYear';
        $this->lang = true;
        $this->addRowAction('edit');
        $this->addRowAction('delete');
        $this->bulk_actions = array('delete' => array('text' => $this->l('Delete selected'), 'confirm' => $this->l('Delete selected items?'), 'icon' => 'icon-trash'));
        $this->fields_list = array(
            'id_leopartsfilter_year' => array(
                'title' => $this->l('ID'),
                'align' => 'center',
        ...
        $this->_select .= ' cl.name make_name, dl.name model_name';
        ...
        $this->_group = ' GROUP BY (a.id_leopartsfilter_year) ';
...
//            $breadcrumbs2['tab']['name'] = $tabs[0]['name'].'';
            $breadcrumbs2['tab']['name'] = 'Level 3 Management';
        ...
        $this->breadcrumbs[] = 'Level 3 Management';
    ...
    {
        $this->toolbar_title = $this->l('Level 3 Management');
    ...
    public function renderForm()
    {
        ...
        $current_link = Tools::getHttpHost(true).__PS_BASE_URI__ . $this->mod_config->getConfig('PS_MMY_FILTER_URL_'.Configuration::get('PS_LANG_DEFAULT'), false, false);
...
//                    'hint' => $this->l('Invalid characters:').' &lt;&gt;;=#{}'
        ...
        $form['buttons']['save'] = array(
            'name' => 'submitAddleopartsfilter_year',
            'type' => 'submit',
            'title' => $this->l('Save'),
            'class' => 'btn btn-default pull-right',
            'icon' => 'process-icon-save'
    ...
    public function processAdd()
    {
        ...
        if (count($this->errors) <= 0) {
            $mod_model = new LeopartsfilterYear();
            $this->copyFromPost($mod_model, 'year');
...
//            $mod_model->date_add = date('Y-m-d', time());
    ...
    protected function _childValidation()
    {
        ...
        foreach ($languages as $language) {
            ...
            $id_year = Tools::getValue('id_leopartsfilter_year');
            if (!empty($value)) {
                $exist = $mod_year->existName($value, $id_model, $id_year, $language['id_lang']);
                if ($exist) {
